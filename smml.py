...
import torch
import math
from layers import general_conv3d_prenorm, fusion_prenorm, sam_weight_fuse, region_aware_modal_fusion
...
class UNet3D(nn.Module):
    def __init__(self, in_channels, out_channels, filters=(32, 64, 128, 256)):
        ...
        self.encoder2 = self.conv_block(filters[0], filters[1], stride=2)
        self.encoder3 = self.conv_block(filters[1], filters[2], stride=2)
        self.encoder4 = self.conv_block(filters[2], filters[3], stride=2)

        # Decoder
        self.upconv3 = self.upconv(filters[3], filters[2])
        self.decoder3 = self.conv_block(filters[2] * 2, filters[2])
        self.upconv2 = self.upconv(filters[2], filters[1])
        self.decoder2 = self.conv_block(filters[1] * 2, filters[1])
        self.upconv1 = self.upconv(filters[1], filters[0])
        self.decoder1 = self.conv_block(filters[0] * 2, filters[0])

        self.conv_final = nn.Conv3d(filters[0], out_channels, kernel_size=1)
    ...
    def conv_block(self, in_channels, out_channels, stride=1):
        return nn.Sequential(
            nn.Conv3d(in_channels, out_channels, kernel_size=3, padding=1, stride=stride, bias=False),
...
class Model(nn.Module):
    def __init__(self, num_cls=4):
        ...
        ### sam
        self.unet3d = UNet3D(in_channels=5*num_cls, out_channels=num_cls)
        self.sam_weight_fuse = sam_weight_fuse(num_cls=num_cls)
        ###
        for m in self.modules():
            if isinstance(m, nn.Conv3d):
                torch.nn.init.kaiming_normal_(m.weight) #

    def forward(self, x, mask, sam_target=None, gt=None):
        ...
        # print(flair_token_x5.shape, self.flair_pos.shape)
        ...
        if self.is_training:
            '''yao'''
            sam_target = self.sam_masker(sam_target, mask, num_cls=self.num_cls) # b,m*k,h,w,z
          
            sam_target_weight = self.sam_weight_fuse(sam_target, gt)
            sam_fusion_pred = torch.cat((sam_target_weight, fuse_pred.detach()), dim=1)  # b,(m+1)k,h,w,z
            sam_pred = self.unet3d(sam_fusion_pred)
            
            '''yao'''
            return fuse_pred, (flair_pred, t1ce_pred, t1_pred, t2_pred), preds, x5_inter, sam_pred
        return fuse_pred
    
    def sam_masker(self,x, mask,num_cls=4):
        B, KM, H, W, Z = x.size()
        x = x.reshape(B,num_cls, -1, H,W,Z).permute(0,2,1,3,4,5).contiguous()
        y = torch.zeros_like(x)
