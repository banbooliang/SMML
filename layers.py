...
class fusion_prenorm(nn.Module):
    def __init__(self, in_channel=64, num_cls=4):
        ...
        self.fusion_layer = nn.Sequential(
                        general_conv3d_prenorm(in_channel*4, in_channel, k_size=1, padding=0, stride=1),
...
class sam_weight_fuse(nn.Module):
    def __init__(self, num_cls=4):
        super(sam_weight_fuse, self).__init__()
        self.num_cls = num_cls
        self.weight_fusion = nn.ModuleList([sam_weight_f() for i in range(num_cls)])
        self.sigmoid = nn.Sigmoid()

    def forward(self, x, gt):
        B,MK,H,W,Z = x.shape
        x = x.reshape(B,-1,self.num_cls,H,W,Z).permute(0,2,1,3,4,5).contiguous()  # B,k,M,H,W,Z
        
        region_fused_pred = []
        for i in range(self.num_cls):
            region_fused_pred.append(self.weight_fusion[i](x[:,i,...], gt[:, i:i+1, ...]))
        region_fused_pred = torch.stack(region_fused_pred, dim=1).permute(0,2,1,3,4,5).contiguous() # b,Mk,h,w,z
        region_fused_pred = region_fused_pred.view(B,-1,H,W,Z)
        return region_fused_pred


class sam_weight_f(nn.Module):
    def __init__(self):
        super(sam_weight_f, self).__init__()
        self.weight_layer = nn.Sequential(
                            nn.Conv3d(4+1, 64, 1, padding=0, bias=True),
                            nn.LeakyReLU(negative_slope=0.2, inplace=True),
                            nn.Conv3d(64, 128, 1, padding=0, bias=True),
                            nn.LeakyReLU(negative_slope=0.2, inplace=True),
                            nn.Conv3d(128, 64, 1, padding=0, bias=True),
                            nn.LeakyReLU(negative_slope=0.2, inplace=True),
                            nn.Conv3d(64, 4, 1, padding=0, bias=True))
        self.sigmoid = nn.Sigmoid()

    def forward(self, x, prm):
        B, M, H, W, Z = x.size()
        prm_avg = torch.mean(prm, dim=(2,3,4), keepdim=False) + 1e-7  # B, 1,
        feat_avg = torch.mean(x, dim=(2,3,4), keepdim=False) / prm_avg  # B, M,
        feat_avg = feat_avg.view(B, M, 1, 1, 1)
        feat_avg = torch.cat((feat_avg, prm_avg[:, 0, ...].view(B, 1, 1, 1, 1)), dim=1)
        weight = torch.reshape(self.weight_layer(feat_avg.float()), (B, M, 1))
        weight = self.sigmoid(weight).view(B, M, 1, 1, 1)
        region_feat = x * weight
        return region_feat
